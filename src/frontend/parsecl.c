...
extern uint8_t bootdrive, ethif, verbose, cgaonly, *biosfile, usessource,
    noscale, nosmooth, renderbenchmark, useconsole, doaudio;
...
void showhelp() {
  ...
  printf("  -fd0 filename    Specify a floppy disk image file to use as floppy "
         "0.\n");
  printf("  -fd1 filename    Specify a floppy disk image file to use as floppy "
         "1.\n");
  printf("  -hd0 filename    Specify a hard disk image file to use as hard "
         "drive 0.\n");
  printf("  -hd1 filename    Specify a hard disk image file to use as hard "
         "drive 1.\n");
  printf("  -boot #          Specify which BIOS drive ID should be the boot "
         "device in #.\n");
  printf("                   Examples: -boot 0 will boot from floppy 0.\n");
  printf("                             -boot 1 will boot from floppy 1.\n");
  printf(
      "                             -boot 128 will boot from hard drive 0.\n");
  printf("                             -boot rom will boot to ROM BASIC if "
         "available.\n");
  printf("                   Default boot device is hard drive 0, if it "
         "exists.\n");
  printf("                   Otherwise, the default is floppy 0.\n");
  printf("  -bios filename   Specify alternate BIOS ROM image to use.\n");
#ifdef NETWORKING_ENABLED
#ifdef _WIN32
  printf("  -net #           Enable ethernet emulation via winpcap, where # is "
         "the\n");
#else
  printf("  -net #           Enable ethernet emulation via libpcap, where # is "
         "the\n");
#endif
  printf("                   numeric ID of your host's network interface to "
         "bridge.\n");
  printf("                   To get a list of possible interfaces, use -net "
         "list\n");
#endif
  printf("  -nosound         Disable audio emulation and output.\n");
  printf("  -fullscreen      Start Fake86 in fullscreen mode.\n");
  printf("  -verbose         Verbose mode. Operation details will be written "
         "to stdout.\n");
  printf("  -delay           Specify how many milliseconds the render thread "
         "should idle\n");
  printf("                   between drawing each frame. Default is 20 ms. "
         "(~50 FPS)\n");
  printf("  -slowsys         If your machine is very slow and you have audio "
         "dropouts,\n");
  printf("                   use this option to sacrifice audio quality to "
         "compensate.\n");
  printf("                   If you still have dropouts, then also decrease "
         "sample rate\n");
  ...
  printf("  -latency #       Change audio buffering and output latency. "
         "(default: 100 ms)\n");
  printf("  -samprate #      Change audio emulation sample rate. (default: "
         "48000 Hz)\n");
  printf("  -console         Enable console on stdio during emulation.\n");
  printf("  -oprom addr rom  Inject a custom option ROM binary at an address "
         "in hex.\n");
  printf("                   Example: -oprom F4000 monitor.bin\n");
  printf("                            This loads the data from monitor.bin at "
         "0xF4000.\n");
...
void parsecl(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    printf("Invoke Fake86 with the parameter -h for help and usage "
           "information.\n");
#ifndef _WIN32
    exit(0);
#endif
