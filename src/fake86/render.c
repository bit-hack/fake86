...
uint8_t initscreen(uint8_t *ver) {
  ...
  thread_video = SDL_CreateThread(VideoThread, NULL);
  if (!thread_video) {
    return 0;
  }
...
uint32_t nw,
    nh; // native width and height, pre-stretching (i.e. 320x200 for mode 13h)
void createscalemap() {
  ...
  if (scalemap != NULL)
    free(scalemap);
  scalemap =
      (void *)malloc(((uint32_t)screen->w + 1) * (uint32_t)screen->h * 4);
  if (scalemap == NULL) {
...
//			if (usefullscreen) HideMenu(); else ShowMenu();
    ...
    if (constantw && constanth)
      screen = SDL_SetVideoMode(constantw, constanth, 32,
                                SDL_HWSURFACE | usefullscreen);
    else if (noscale)
...
void stretchblit(SDL_Surface *target) {
  ...
  for (dsty = 0; dsty < (uint32_t)target->h; dsty++) {
    ...
    for (dstx = 0; dstx < (uint32_t)target->w; dstx++) {
      ...
      ((uint32_t *)target->pixels)[ofs++] =
          SDL_MapRGB(target->format, (uint8_t)r, (uint8_t)g, (uint8_t)b);
      lastx = srcx;
    }
    lasty = srcy;
  }
...
void roughblit(SDL_Surface *target) {
  ...
  for (dsty = 0; dsty < (uint32_t)target->h; dsty++) {
    ...
    for (dstx = 0; dstx < (uint32_t)target->w; dstx++) {
      ...
      pixelrgb = (uint8_t *)&prestretch[srcy][srcx];
      ((uint32_t *)target->pixels)[ofs++] =
          SDL_MapRGB(target->format, pixelrgb[0], pixelrgb[1], pixelrgb[2]);
    }
  }
...
/* NOTE: doubleblit is only used when smoothing is not enabled, and the SDL
   window size
         is exactly double of native resolution for the current video mode. we
   can take
         advantage of the fact that every pixel is simply doubled both
   horizontally and
         vertically. this way, we do not need to waste mountains of CPU time
   doing
         floating point multiplication for each and every on-screen pixel. it
   makes the
         difference between games being smooth and playable, and being jerky on
   my old
...
void draw() {
  uint32_t planemode, vgapage, color, chary, charx, vidptr, divx, divy, curchar,
      curpixel, usepal, intensity, blockw, curheight, x1, y1;
  switch (vidmode) {
  case 0:
  case 1:
  case 2: // text modes
      ...
      for (x = 0; x < 640; x++) {
        ...
        if (vidcolor) {
          /*if (!color) if (portram[0x3D8]&128) color = palettecga[ (RAM[vidptr+1]/16) &7];
              ...
              !color)
            color =
                palettecga[RAM[vidptr + 1] / 16]; // high intensity background
          else
