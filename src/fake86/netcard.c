...
void nethandler() {
  ...
  case 0x01: // send packet of CX at DS:SI
    ...
    sendpkt(
        &RAM[((uint32_t)segregs[regds] << 4) + (uint32_t)regs.wordregs[regsi]],
        regs.wordregs[regcx]);
    return;
  ...
  case 0x03: // copy packet to final destination (given in ES:DI)
    memcpy(
        &RAM[((uint32_t)segregs[reges] << 4) + (uint32_t)regs.wordregs[regdi]],
        &RAM[0xD0000], net.pktlen);
    return;
  ...
  case 0x05: // DEBUG: dump packet (DS:SI) of CX bytes to stdout
    for (i = 0; i < regs.wordregs[regcx]; i++) {
      printf("%c", RAM[((uint32_t)segregs[regds] << 4) +
                       (uint32_t)regs.wordregs[regsi] + i]);
    }
    return;
