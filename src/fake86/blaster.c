...
extern void outadlib(uint16_t portnum, uint8_t value); // on the Sound Blaster
                                                       // Pro, ports (base+0)
                                                       // and (base+1) are for
extern uint8_t inadlib(uint16_t portnum); // the OPL FM music chips, and are
                                          // also mirrored at (base+8) (base+9)
  ...
  if (blaster.waitforarg) {
    ...
    case 0x91:
      if (blaster.waitforarg == 2) {
        ...
        blaster.blocksize =
            (blaster.blocksize & 0x00FF) | ((uint32_t)value << 8);
#ifdef DEBUG_BLASTER
    ...
    case 0x40: // set time constant
      blaster.samplerate =
          (uint16_t)((uint32_t)1000000 / (uint32_t)(256 - (uint32_t)value));
      setsampleticks();
#ifdef DEBUG_BLASTER
    ...
    case 0x48: // set DSP block transfer size
      ...
      break;
    case 0xE0: // DSP identification for Sound Blaster 2.0 and newer (invert
               // each bit and put in read buffer)
      bufNewData(~value);
      break;
...
#endif
#ifdef DEBUG_BLASTER
// if (lastread != portnum) printf ("[DEBUG] inBlaster: port %Xh, value ",
// portnum);
#endif
  portnum &= 0xF;
  switch (portnum) {
...
void mixerReset() {
  ...
  blaster.mixer.reg[0x22] = blaster.mixer.reg[0x26] = blaster.mixer.reg[0x04] =
      (4 << 5) | (4 << 1);
}
