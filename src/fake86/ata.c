...
void bufwrite8(uint16_t bufpos, uint8_t value) { databuf[bufpos] = value; }
...
uint8_t inATA(uint16_t portnum) {
  ...
  case 0x1F7: // status register
    ...
    if (drivesel == 1)
      statusreg |= ATA_STATUS_ERROR;
    if (dataptr < 512)
      statusreg |= ATA_STATUS_DRQ;
    return (statusreg);
  }
  return (0);
}
